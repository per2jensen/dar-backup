...
- **Explicit in-place overwrite restores** (`dar_backup.py`, `manager.py`, `restore_target_safety.py`, `doc/restoring*.md`) — `--overwrite-restore-target` is now available for both named-archive and PITR restores when a second staged tree will not fit. The default empty/no-overlap policies remain unchanged. Overwrite restores require an explicit target, hold the descriptor-bound target lock, audit the complete existing tree before DAR starts, reject untrusted owners, group/other write access, extended ACLs, selected-path symlinks, and protected locations, and verify that the requested pathname still names the locked inode around every DAR extraction. Normal restores trust only the restoring UID; a root-run restore trusts root and the target-root owner so root can recover a private user-owned home tree. Preflight reports collect distinct blockers and recoverable inspection failures across the tree, stopping at 100 with an explicit warning that more may exist, so operators can remediate a useful bounded batch per attempt. DAR's explicit `Oo` overwrite policy is appended after custom restore options; deletion handling remains controlled independently by `--no-deleted`. Logs announce potentially lengthy preflight work, progress, completed counts, in-place risk, and partial-state recovery. Real integration coverage proves a private fake home is modified in place while unrelated and outside data survive, creates real access/default POSIX ACLs to verify fail-closed preflight behavior, and applies a real FULL → DIFF → INCR PITR chain to populated targets with both deletion policies.
