...
- **CLI reference now combines scan-friendly lists with detailed option behaviour** (`doc/cli-reference.md`, `tests/test_cli_reference.py`) — every option for all eight installed commands now has a dedicated explanation covering defaults, dependencies, conflicts, side effects, and safety constraints where applicable. A section-aware parser/source test verifies that every explicit argparse option, plus automatic help, remains present in both its command's short list and detailed headings; options documented under another command or mentioned only in prose do not satisfy the check.
- **Explicit in-place overwrite restores** (`dar_backup.py`, `manager.py`, `restore_target_safety.py`, `doc/restoring*.md`) — `--overwrite-restore-target` is now available for both named-archive and PITR restores when a second staged tree will not fit. The default empty/no-overlap policies remain unchanged. Overwrite restores require an explicit target, hold the descriptor-bound target lock, audit the complete existing tree before DAR starts, reject untrusted owners, other-write access, unsafe group membership, extended ACLs, selected-path symlinks, and protected locations, and verify that the requested pathname still names the locked inode around every DAR extraction. Owner-only groups are resolved from one immutable NSS snapshot, combining passwd primary-GID and group supplementary membership, so ordinary `<user>:<private-group>` directories with modes `0770`/`0775` pass without permission changes; ambiguous or shared membership fails closed. Root has an explicit `--force-unsafe-restore-target` break-glass option that audits and CRITICAL-logs up to 100 overridable policy findings but never bypasses symlink, target-open/type/lock, pathname-identity, selection, option-injection, or protected-target checks. Expected target refusals now log once without a duplicate “Unexpected error.” DAR's explicit `Oo` overwrite policy is appended after custom restore options; deletion handling remains controlled independently by `--no-deleted`. Real integration coverage proves owner-only group-writable fake homes and PITR trees restore in place while unrelated and outside data survive, creates real access/default POSIX ACLs to verify fail-closed preflight behavior, and applies a real FULL → DIFF → INCR PITR chain to populated targets with both deletion policies.
- **Restore documentation is now operator-first** (`doc/restoring*.md`) — the former 452-line `restoring.md` mixed quick-start commands, target-safety rules, detailed PITR semantics, raw `dar_manager` procedures, archive inspection, and FSA troubleshooting. `restoring.md` is now the concise entry point with a restore-method decision table, an incident-oriented target-preparation checklist, the complete target-safety matrix, refusal-message recovery actions, same-filesystem staging and capacity guidance, explicit-target examples, and post-restore verification. Detailed PITR behavior moved to `restoring-pitr.md`; manual and specialized procedures moved to `restoring-advanced.md`. Wheel staging still uses the existing negative documentation list, and `release.sh` now fails if any member of the three-file restore guide is missing.

### Fixed

- **Release artifacts could lose all packaged documentation before upload** (`release.sh`, `scripts/release_validation.py`) — the full pytest suite's real-document fixture removed the shared `src/dar_backup/doc` staging directory after `release.sh` populated it, so the subsequent wheel silently omitted README, changelog, and user guides; the exit trap then emitted a misleading `rmdir` error. Release staging now happens after tests, cleanup is idempotent, every staged document is verified inside each built wheel, and `twine check` runs before signing. The release environment now requires `packaging>=26.0` and `twine>=6.2.0`, which understand Core Metadata 2.5 emitted by current build backends.
- **Cleanup could exit successfully after file or catalog deletion failures** (`cleanup.py`) — DAR and PAR2 deletion errors and failed catalog removals now propagate to the CLI exit status. Partial multi-slice deletion removes the database entry because the archive is no longer restorable, logs the slices left on disk, and exits nonzero; if no slice could be deleted, the catalog and PAR2 files are retained.
- **Restore-target validation remained vulnerable to pathname replacement races** (`restore_target_safety.py`, `command_runner.py`, `dar_backup.py`, `manager.py`) — the target was resolved, checked, opened, and locked, but DAR later received the original pathname; replacing that pathname or a parent after validation could redirect extraction while the lock remained attached to the old inode. Direct and PITR restores now pass DAR a Linux `/proc/self/fd` root backed by the validated open directory and explicitly inherit that descriptor. Existing-data and selected-path checks are descriptor-relative, target replacement during preparation fails closed, and privileged restores reject targets or traversed existing directories writable by another identity. Real-DAR regression coverage replaces the requested target with an outside symlink after preparation and proves extraction remains in the originally opened directory.
- **Direct archive restores could silently overwrite populated or protected targets** (`dar_backup.py`, `restore_target_safety.py`) — direct `--restore` always passed DAR `-wa`, but unlike PITR it did not validate, lock, or inspect an explicit `--restore-dir`; no `--selection` restored the whole archive and `--selection="-g ."` explicitly selected its root. Both restore paths now use one policy-driven target-safety context that independently controls protected locations and existing-data behavior. By default, direct restores require an empty, non-protected target; PITR root restores require empty targets; PITR specific paths reject only overlaps. Internal policy coverage exercises every protected-target × existing-data combination; protected-target bypass remains dormant, while overwrite is exposed only through the explicit audited option described above. Raw direct selection can no longer inject `-R`, batch files, or command execution.
- **PITR archive-root path `.` bypassed the no-overwrite target guard** (`manager.py`) — dot-equivalent paths were deliberately skipped during overlap checks and then passed to DAR as `-g . -wa`; DAR treats that as a tree selection and silently overwrites matching files throughout a populated target. Archive-root restores remain supported, but now require a completely empty target while the restore lock is held; hidden entries, directories, and dangling symlinks all count as nonempty, and an unreadable target fails closed. Unit coverage includes `.`, `./`, `./.`, protected and allowed target symlinks, and scan failure; a real-DAR regression proves populated-target content remains unchanged and an empty-target root restore succeeds.
- **Timezone-aware historical PITR requests could select the wrong archive date** (`manager.py`) — aware `--when` values reused the UTC offset in effect when the restore ran, so a winter timestamp converted during summer (or vice versa) could move by the wrong amount and cross a local-midnight archive boundary. Conversion now asks the operating system for the local timezone rules at the requested instant, including southern-hemisphere DST, non-hour offsets, and 30-minute DST changes. Deterministic tests cover exact conversions and archive inclusion/exclusion across representative timezone rule shapes.
- **Aborted or verification-failed backups could be published as PITR restore points** (`dar_backup.py`) — DAR exit code 4 means the operation was aborted and its archive may be incomplete, but it was accepted for manager registration alongside codes 0 and 5; if later verification passed, the run could even exit 0 and report success. Catalog registration also preceded verification, and verification failure never removed the entry. Code 4 is now a hard DAR-phase failure that leaves any partial slices uncataloged, and registration has moved after successful archive/restore verification so rejected archives are never visible to PITR. Verified archives are still registered before PAR2 because redundancy generation does not determine archive validity. Real-process regression coverage creates a valid archive through DAR and then forces exit 4, proving that status alone prevents publication; the existing real restore-verification failure test now also asserts `catalog_updated=0` and an absent manager entry.
- **Single-file PITR resurrected paths deleted or renamed away by an eligible DIFF/INCR** (`manager.py`) — `dar_manager -f` parsing retained only `saved` entries, so a later `removed` tombstone was discarded and PITR selected stale bytes from an older FULL with exit code 0. File selection now evaluates the newest eligible `saved`/`present`/`removed` state before choosing an archive: `removed` returns non-zero without invoking dar, `present` resolves to the nearest earlier saved data without crossing a removal boundary, and a later `saved` entry correctly represents a recreated path. Unknown or malformed numbered status rows fail closed. Real-DAR coverage pins explicit deletion, both sides of a rename, report modes, and the archive-date captured-state boundary.
- **PITR could exit successfully after an incomplete multi-slice restore** (`manager.py`, `util.py`) — availability checks looked only for `<archive>.1.dar`, while `dar -x -Q` can return 0 after silently skipping data from a missing interior slice. PITR report, report-first, and direct restore now require a contiguous slice sequence beginning at 1 and use a filtered DAR catalogue read to prove the highest available slice is the archive's actual final slice. Directory chains are fully preflighted before their FULL is applied, and slice filenames are checked again around extraction so concurrent slice loss cannot produce manager exit code 0. Real-DAR tests cover complete archives, missing interior slices, missing final slices, and the original zero-exit/incomplete-data behavior.
- **PITR `--restore-path` accepted absolute paths, `..` components, and symlinks planted inside the restore target** (`manager.py`) — nothing validated `--restore-path` values before they reached `dar -g`; an absolute path was silently accepted but only *partially* normalized (stripped for the local pre-existence check, passed unmodified to the actual `dar` invocation). More seriously, if a restore target directory was reused or otherwise writable by another party, a symlink planted at any path component — including a *dangling* leaf symlink, which `os.path.exists()` reports as absent — would not be caught by the existing-path guard and could redirect dar's writes outside the target. `--restore-path` values are now validated up front (relative only, no `..` component, non-empty) in both `restore_at()` and `--pitr-report`, and every path component inside the target is checked for symlinks (dangling or not) immediately before restoring. Unit tests cover absolute/`..`/empty paths and both a mid-path and a dangling-leaf symlink; the symlink tests were confirmed to fail against the unguarded code before the fix.
- **PITR catalog-fallback directory detection inspected only the newest FULL in the catalog, not the one `--when` actually selects** (`manager.py`) — `_detect_directory()`'s fallback (used when a restore path no longer exists on disk — the disaster-recovery case) always inspected the most recent FULL archive. Restoring an older directory that was deleted before a newer FULL ran (or one created only in a DIFF/INCR) would miss it there, misclassify the path as a file, and restore from a single archive instead of the FULL→DIFF→INCR chain — silently dropping files with a success exit code. `_detect_directory()` now inspects the chain selected for `--when` (newest chain member first), falling back to the newest FULL only when no FULL exists at or before `--when`. A real integration test (directory deleted before a newer FULL, restored to an earlier point in time) fails against the old behavior and passes now.
- **PITR archive-listing and file-lookup failures could silently use partial or misleading results** (`manager.py`) — `dar_manager --list` and `dar_manager -f` return codes were never checked before parsing their output. A listing that failed partway through (corrupt DB page, I/O error) could still emit a parseable but truncated archive list, letting PITR believe an older archive was the newest and select the wrong version. Separately, a failed `-f` lookup with empty output was indistinguishable from "file has no version at or before `--when`" — including an empirically-confirmed case where a corrupted database makes `dar_manager -f` exit 0 with "Corrupted database" text instead of a nonzero code. `--list` return codes are now checked in both `_pitr_chain_report()` and `_restore_with_dar()`; `-f` lookups distinguish the benign "not in catalog" case (rc=2, "Non existent file in database") from real failures via error markers, surfacing a "Version lookup failed" error instead of a misleading "no version found".
- **PITR misclassified private directories (700/750/770) during catalog-fallback detection** (`manager.py`) — `_is_directory_in_archive()` matched the `dar -l` permission field with a regex ending in `\b`, but a permission string ending in `-` (e.g. `drwx------` for mode 700 — the classic case is `~/.ssh`) has no word boundary before the following whitespace, so exactly the private directories never matched. In the disaster-recovery path (directory deleted from disk, detection falls back to `dar -l`), such a directory was treated as a *file* and restored from a single archive instead of the FULL→DIFF→INCR chain — silently dropping files with a success exit code. The permission field is now matched by its whitespace delimiters (tolerating a trailing ACL `+` marker). Unit tests use verbatim `dar -l` output for 700/750 directories; a real integration test (0700 directory + DIFF chain) fails against the old regex and passes now.
- **PITR file restore silently fell back to an older version when dar failed** (`manager.py`) — when the best candidate's slice existed on disk but dar failed to extract it (e.g. a corrupt or truncated slice), `_restore_with_dar()` retried successively older catalog versions and exited 0 if any of them restored — delivering stale data with only an ERROR line in the log. Missing archives already failed fast; a failing dar now does too: only the newest version at or before `--when` is ever tried, and any failure fails the restore. The ERROR output lists the older catalog versions with the timestamps a rerun's `--when` must target, and points to par2 repair as the first recovery step; `doc/restoring.md` documents the full recovery sequence ("When the selected archive is damaged").
...
- **PITR single-file restores now follow the archive-date contract** (`manager.py`) — file restores previously selected by the recorded file *mtime* from `dar_manager -f`, contradicting the documented contract ("archive creation date ≤ --when") and the directory branch. Consequences of the old behavior: a rename or edit captured in a later DIFF could be restored for a `--when` before that DIFF existed (mtime predates the archive), and restoring a file vs. its parent directory at the same `--when` could return different content. Files are now restored from the newest archive created at or before `--when` that *saved* the file's data (`dar_manager -f` only identifies which archives hold it; entries where an unchanged file is merely "present" without data are excluded); same-date archives tie-break by backup order (FULL < DIFF < INCR), matching chain selection. If a catalog number recording the file cannot be resolved to a dated archive, the restore fails fast instead of guessing. Note: files now share the documented same-day FULL+DIFF date-granularity limitation. Users who want mtime-based version picking keep the documented `dar_manager -w` path (restoring.md, "Restore a file by its mtime"). Integration tests pin the edit trap, the rename trap, and post-DIFF selection with time-suffixed archives.
...
- **Explicit `mypy src/` gate added directly to `release.sh` and `.github/workflows/py-tests.yml`** — `release.sh` now runs `mypy src/` immediately after the existing `ruff check src/` gate, before the version/tag logic and the (much slower) full test suite; both workflow jobs (`smoke-tests`, `full-tests`) in `py-tests.yml` gained a dedicated "Run mypy" step right after the venv build and before the pytest step. Both of these duplicate a check that already runs inside `scripts/pytest_report.sh` (called later by both `release.sh` and the workflow's own pytest step) — not new coverage, but a deliberate fail-fast: a type regression is now caught in a couple of seconds by its own clearly-named step, instead of surfacing after the test suite has already spent 10-20+ minutes running, buried inside a differently-named step's log.

- **INCR phase and byte-level restore verification in `large_scale_test.sh`** (`scripts/large_scale_test.sh`) — the torture test previously only exercised FULL → DIFF → restore-latest, leaving INCR — arguably the riskiest tier, since it depends on correctly chaining off the latest DIFF — completely untested at scale.  Added `Phase 2c`, mirroring the existing DIFF pattern: mutates the diff-primer data again (removes one of the two current hardlink names, adds a new one, adds a new file), takes an INCR backup, and verifies its contents the same way DIFF is verified (`verify_incr_contents`).  Bitrot injection + PAR2 repair (previously FULL-only) now also runs against the DIFF and INCR archives.  The Phase 3a restore-to-latest check now covers the full 3-tier hardlink chain and additionally captures sha256 checksums of the final primer state immediately before restoring, then verifies every restored file matches byte-for-byte (`capture_primer_checksums` / `verify_primer_checksums`) — proving a restore reproduces exactly what was backed up, not just that a same-named file appeared.  Historical PITR (restore to a point *between* same-day archives) is deliberately still out of scope for this script: `_select_archive_chain` compares archives by calendar date only, so two archives created in the same script run are indistinguishable to chain selection regardless of real elapsed time — already documented in `doc/pitr-archive-date-vs-file-mtime.md`'s "Known limitation: same-day FULL and DIFF" section. That correctness case is covered by `tests/test_pitr_integration.py`, which controls the timeline directly instead of depending on a real clock.

- **Disk-space preflight in `large_scale_test.sh`** (`scripts/large_scale_test.sh`) — parses the `-R`/`-g` paths out of the generated backup definition, estimates source data size via `du -sb`, and fails fast with a clear message if free space under `--base` is below `--min-free-multiplier` (default `2`) times that estimate — before a multi-hour run can discover "disk full" partway through instead of in the first few seconds.

- **Regression detection in `large_scale_test.sh`** (`scripts/large_scale_test.sh`) — `write_json_record()` now loads the last 5 passing runs from `large-scale-results.jsonl` before appending the new one, and prints a `WARN` line (non-blocking, not counted in `$FAILURES`) if this run's FULL elapsed time or any tool's peak memory exceeds 1.5x the trailing average.  Deliberately a "look at this before tagging a release" signal rather than a hard gate — real hardware/environment variance makes a strict pass/fail too noisy here.

- **`--smoketest` flag** (`scripts/large_scale_test.sh`) — skips mirroring the run's JSONL record into the tracked `doc/test-report/large-scale-results.jsonl`, so a fast/small verification run (e.g. a synthetic dataset used to sanity-check a script change) doesn't pollute the real historical trend data — the same data the regression check above relies on — with unrepresentative numbers.

- **Run-configuration dump in `large_scale_test.sh`** (`scripts/large_scale_test.sh`) — every top-level variable (CLI options, tool versions detected by `preflight()`, and every derived path under `RUN_DIR`/`RESULTS_DIR`) is now documented in place and printed by `print_run_variables()` under a "Run configuration" banner at the very start of each run, captured in the tee'd `$SUMMARY` file.  Motivated by a real case: after a real run without `--keep`, there was no record of which paths had actually been used, since the run directory was already gone by the time anyone wanted to inspect the archives.

- **`BLE` (blind-except), `TRY` (exception-handling hygiene), `DTZ` (naive datetime), and `S` (flake8-bandit security) ruff rule categories enabled** (`pyproject.toml`) — previously only `E`/`F`/`W`/`UP`/`B` were checked, leaving this project's own exception-handling conventions (never silently swallow, catch specific exceptions), security-sensitive subprocess/network/SQL usage, and naive-datetime correctness entirely unenforced by lint. All 410 resulting violations were audited site-by-site rather than blanket-suppressed: genuine issues were fixed (see Fixed, below); the remainder are legitimate patterns given a `# noqa: <CODE>` plus a one-line reason (e.g. top-level CLI-boundary catches that already log and exit, internal invariant `assert`s, PREREQ/POSTREQ's intentional `shell=True`, Jinja2 templates that render plain text rather than HTML). Three sub-rule groups were deliberately deferred alongside the existing `UP` typing-modernization deferral — `G004` (f-string logging, ~160 sites, high effort/risk for a mostly micro-optimization benefit), `TRY003`/`TRY300`/`TRY301` (pure style preferences, no correctness value), and `DTZ007`/`DTZ901` (archive-date parsing is deliberately naive/date-only by design) — see `doc/todo.md`'s "Ruff finding" section.
...
- **RSS monitor simplified in `large_scale_test.sh`** (`scripts/large_scale_test.sh`) — replaced a `pgrep -P`-based process-tree walk (hardcoded to two levels deep: children, then grandchildren) with a single system-wide `ps -eo pid,comm,rss,vsize` scan filtered by the same `dar`/`dar-backup`/`par2`/`manager` command-name pattern already used.  Simpler, and no longer blind to a descendant process one level deeper than the walk covered.
...
- **Logging setup deduplicated** — four identical logging-setup blocks in `dar_backup.py`, `manager.py`, `cleanup.py`, and `installer.py` consolidated into a single `init_logging()` helper in `util.py`.
- **Config attribute access simplified** — scattered `getattr(config_settings, "field", default)` calls replaced with direct `config_settings.field` access throughout; `ConfigSettings` already declares all optional fields with proper defaults so the defensive fallbacks were redundant.
- **PITR directory-chain selection deduplicated** — `_pitr_chain_report()` and `_restore_with_dar()` both performed the same "select archive chain → check empty → check missing slices" sequence independently. Extracted into `_resolve_directory_chain(archive_info, when_dt, archive_map)` which returns `(chain, missing)`; both callers now use the shared helper. Three new unit tests cover the no-FULL, missing-slice, and all-present cases.
- **Command failure logging standardised** — `stderr: …` / `stdout: …` error lines after failed `runner.run()` calls were duplicated 9 times across `manager.py` with inconsistent verbosity (some logged both streams, some only stderr, some guarded on non-empty, some did not). Extracted into `_log_command_failure(result, context)` which logs the context message at ERROR level then emits stderr and stdout at ERROR level only when non-empty. `remove_specific_archive` gained an explicit context message it previously lacked.
- **Archive name parsing unified** — `cleanup.py`, `manager.py`, `dar_backup.py`, and `util.py` each had their own ad-hoc date/definition extraction from archive names (split+strptime, regex groups, `split('_')[0]`). All sites now use `util.ArchiveName.parse()` / `ArchiveName.from_filename()` and `as_datetime()`. `add_specific_archive()` in `manager.py` now rejects archive names that don't match the naming convention with a clear error rather than silently proceeding with a wrong backup definition. `add_directory()` now handles archives with time-suffixed names (e.g. `_143022`) that the old regex silently dropped.
- **Directory validation unified** — the exists → is\_dir → is\_writable check trio was duplicated across `manager.py:create_db()`, `dar_backup.py:preflight_check()`, and `cleanup.py:main()`. Extracted into `util.validate_directory(path, name, require_write=True)` which returns `None` on success or a descriptive error string; each caller handles failure in its own way (return 1, append to errors list, or `sys.exit`). Also fixes a bug in `cleanup.py` where the "not a directory" error omitted the path from the message.
- **Archive slice existence centralised** — `os.path.exists(f"{path}.1.dar")` appeared 6 times across `manager.py` and `dar_backup.py`. Extracted into `util.archive_exists(base_path)` so the `.1.dar` first-slice naming convention is encoded in one place.
- **Dry-run file deletion deduplicated** — the `if dry_run: log / else: safe_remove_file + log` block was repeated three times in `cleanup.py` (`_delete_par2_files`, `delete_old_backups`, `delete_archive`) with slightly inconsistent message formats. Extracted into a single `_remove_file(file_path, base_dir, label, dry_run)` helper that standardises all four outcomes (dry_run / deleted / unsafe / error) and returns a bool; callers use the return value to decide whether to proceed with catalog deletion.
- **Config option parsing centralised** — the `has_option → get → strip → converter → except` pattern was duplicated across `_get_optional_int`, `_get_optional_str`, and the `OPTIONAL_CONFIG_FIELDS` loop in `config_settings.py`. Extracted into `_get_config_value(section, key, converter, default)` which all three now delegate to. `get_par2_config()` replaced its hand-rolled item-iteration loop with explicit `_get_config_value` / `_get_optional_bool` calls, eliminating three repeated `try: int(value)` blocks and two repeated boolean-parsing blocks.
- **Ownership flag resolution centralised** — the `if preserve_ownership / elif ignore_ownership / else: not config.restore_ownership` block was duplicated in `manager.py:main()`, `dar_backup.py:verify()`, and `dar_backup.py:main()`. Extracted into `util.resolve_ownership_flag(args, config_settings)` which uses `getattr` for safe attribute access regardless of whether the CLI args namespace guarantees the fields.
...
- **PITR restore safety check could be bypassed via a symlink, allowing restore into protected system directories** (`manager.py`) — `_restore_target_unsafe_reason()` used `os.path.abspath()` to resolve the target path, which expands `.` and `..` but does **not** follow symlinks.  A symlink at e.g. `/home/user/link` pointing to `/etc` appeared to be under `/home/` and was allowed; dar would then restore files directly into `/etc`.  Fixed by replacing `abspath()` + `normpath()` with a single `os.path.realpath()` call, which follows the full symlink chain to the canonical path before the allow/protect check runs.  This is consistent with how the rest of `manager.py` resolves paths (lines 63, 961, 1178).  Two new tests cover the bypass case (symlink into `/etc` is now blocked) and confirm that symlinks to genuinely safe locations are not false-positived.

- **PREREQ/POSTREQ timeout could hang the process indefinitely when a script spawned background children** (`util.py`) — `subprocess.Popen` was called without `start_new_session=True`, so background children spawned by a script (e.g. `cmd &`) inherited the stdout/stderr pipe file descriptors from the shell.  After the shell exited (or was killed on timeout), those children kept the pipes open, causing the reader-thread `join()` calls to block indefinitely.  Fixed by adding `start_new_session=True` so the shell and all its descendants form a new process group; the process group ID is now captured immediately after `Popen` (before `wait()` reaps the shell PID), and `os.killpg()` is called both on timeout and after normal exit to reap any lingering background children before joining threads.  Thread joins now have a 5-second safety timeout.  Two new unit tests cover the background-child no-hang case and the timeout-raises-promptly case.

- **PAR2 generation gave no diagnostic when it failed mid-run** (`dar_backup.py`) — if `par2 create` failed on slice N of M, the exception propagated silently with only a per-slice error log.  Operators had no summary of how many slices had been protected before the failure.  Each slice's par2 set is self-contained (slice 1's par2 can repair slice 1 independently of whether slice 2 has par2), so the par2 files already written for completed slices are retained — partial coverage is better than none.  The fix wraps the per-slice loop in `try/except`: on failure, if at least one slice completed, a `WARNING` is logged stating exactly how many slices have par2 coverage (e.g. "PAR2 generation failed after 3/10 slice(s) … par2 files for the first 3 slice(s) remain on disk and provide partial recovery coverage").  If the very first slice fails (zero coverage), no coverage warning is emitted.  Two new tests verify: (1) par2 files for completed slices are kept on disk and the coverage warning is logged, (2) no spurious coverage warning appears when zero slices completed.

- **`install_autocompletion()` left the RC file handle unclosed after appending** (`installer.py`) — `rc_file.open("a").write(...)` returned a file object that was written to and immediately discarded without calling `close()`.  CPython closes unclosed handles via reference counting when the object is garbage collected, but this is not guaranteed in alternative Python implementations and is inconsistent with the `with`-statement pattern used everywhere else in the codebase.  Fixed by wrapping the append in a `with rc_file.open("a") as fh:` block.  One new unit test instruments the real file handle's `close()` method to verify it is called explicitly by the `with` statement rather than by the GC.

- **`DoctypeStripper` had no context-manager support — file handle leaked on parse error** (`dar_backup.py`) — `DoctypeStripper` opens a file handle in `__init__` but had no `close()`, `__enter__`, or `__exit__`.  It was passed directly to `ET.iterparse()` without a `with` statement.  If the parser raised mid-parse (malformed XML, I/O error, XXE `ParseError`), the `DoctypeStripper` instance became orphaned and `self.f` was left open until the GC ran — nondeterministic in CPython and not guaranteed in other implementations.  For a long-running backup process verifying many archives this could accumulate open file descriptors until the OS limit was hit.  Fixed by adding `close()`, `__enter__`, and `__exit__` to `DoctypeStripper` and wrapping the `ET.iterparse()` call in `iter_files_with_paths_from_xml()` with a `with` statement.  Using `with` inside a generator correctly handles all three cases: normal exhaustion (exits the block), parse error (exception propagates through `__exit__`), and abandoned iteration (`GeneratorExit` is thrown into the generator at the `yield` point and propagates through the `with` block).  Three new unit tests cover: normal-exit close, exception-exit close, and close-on-ParseError via `iter_files_with_paths_from_xml()`.

- **`_size_in_verification_range()` duplicated the dar unit table from `_parse_size_bytes()`, creating a divergence risk** (`dar_backup.py`) — both functions maintained an identical `dict` mapping dar size units (`o`, `kio`, `Mio`, `Gio`, `Tio`) to byte multipliers.  If a new unit were added to the module-level `_DAR_SIZE_UNITS` table (e.g. `Pio` for pebibytes), `_parse_size_bytes()` would handle it correctly but `_size_in_verification_range()` would silently reject those files, causing them to be dropped from the restore-test sample pool with no error.  Fixed by removing the local dict and delegating entirely to `_parse_size_bytes()`: a `None` return maps to `False`, and the range check runs on the returned byte count.  Five new unit tests cover in-range, below-range, above-range, unparseable input, and the unknown-unit delegation case that directly guards against future divergence.

- **Subprocess spawn failures in `generic_backup()` and `verify()` were silently discarded on systemd** (`dar_backup.py`) — both functions had a bare `print(f"[!] Backup failed: {e}")` in the `except` block that catches `runner.run()` raising (e.g. the process could not be spawned).  On a systemd service stdout is not captured, so this message produced no log file entry and no journal record — the only observable signal was the exception propagating up.  Additionally, the message in `verify()` said "Backup failed" even though the failing operation was the archive integrity test, making triage harder.  Fixed by replacing both `print()` calls with `logger.error()` carrying the correct operation name: `generic_backup()` now logs "Backup command could not be run for …" and `verify()` logs "Verification command could not be run for …".  Two new component tests assert `logger.error` is called with the correct operation name when `runner.run()` raises.

- **Missing archive slice or par2 file after successful backup caused a false FAIL** (`dar_backup.py`) — after dar completed, `_list_dar_slices()` enumerated `.dar` slices and then `os.path.getsize()` was called on each; similarly, after par2 generation, `glob.glob()` enumerated `.par2` files and `os.path.getsize()` was called on each.  In both cases an unhandled `OSError` (e.g. a file that disappeared between the enumeration and the stat call) propagated to the outer `except Exception` and marked the entire backup as `FAILED`.  The two cases have different semantics and are fixed differently: (1) a missing `.dar` slice means the archive is genuinely incomplete — the `OSError` is now caught, an `ERROR` is logged clearly naming the missing slice (so blame does not fall on the dar phase, which already exited with code 0), and then re-raised so the backup is still recorded as `FAILURE`; (2) a missing `.par2` file after successful generation is a degraded but not data-loss state — par2 files are now measured individually so that the partial sum of available files is still recorded (e.g. 9 out of 10 files present still yields a meaningful metric), each missing file is logged as a `WARNING` naming the specific file, and the backup completes as `SUCCESS`.  Three new component tests cover: the dar-slice missing case, the all-par2-files-missing case (`par2_size_bytes = NULL`), and the partial-par2 case (partial sum stored, missing file named in warning).  Monkeypatched `os.path.getsize` used per project convention for OS-level TOCTOU failures.

- **Stale restore file in the verify phase caused a false FAIL** (`dar_backup.py`) — when a file already existed at the restore target path, `os.remove()` was called before the dar restore.  If `os.remove()` raised `OSError` (e.g. read-only mount, wrong ownership), the exception was silently swallowed with a bare `except OSError: pass`.  The stale file remained on disk and the dar restore command did not include `-wa`, so dar refused to overwrite the existing file and returned non-zero — marking the sample as `FAIL` even though the archive was intact.  Fixed by logging the `OSError` as a `WARNING` (so operators can diagnose permission problems in the restore directory) and adding `-wa` to the dar restore command so the file is overwritten regardless of whether the prior removal succeeded.  This is now consistent with `restore_backup()` and `_restore_with_dar()`, which already passed `-wa`.  Two new tests cover the warning-is-logged path (monkeypatched `os.remove` per project convention for OS-level failures) and assert that `-wa` is present in the captured dar command.

- **`--remove-specific-archive` could silently exit 0 on failure when `manager.py` was invoked directly** (`manager.py`) — every branch in `main()`'s dispatch calls `sys.exit(<fn>(...))` except this one, which did a bare `return remove_specific_archive(...)`.  The installed `manager` console-script wrapper happens to call `sys.exit(main())`, masking the bug in normal use, but a direct `python -m dar_backup.manager --remove-specific-archive ...` invocation always exited 0 regardless of outcome.  Fixed to match every sibling branch: `sys.exit(remove_specific_archive(...))`.  A new test invokes the module directly (bypassing the console-script wrapper) and asserts the exit code is propagated correctly.

- **`clean-log` used a fixed, predictable temp-file path with no cleanup on failure** (`clean_log.py`) — `log_file_path + ".tmp"` was used instead of `tempfile.mkstemp()`, so two concurrent invocations on the same log file (a manual run overlapping a scheduled one) could race on the same temp path, and an exception between opening the temp file and `os.replace()` left a stray `.tmp` file behind.  Fixed by generating the temp file with `tempfile.mkstemp()` in the same directory (required for `os.replace()` to stay atomic) and wrapping the write in `try/finally` so the temp file is removed unless the replace succeeded — mirroring the existing pattern in `util.patch_config_file()`.  Two new tests cover: no stray temp file after a successful clean, and cleanup plus an untouched original file when `os.replace()` fails mid-operation.

- **`ruff check src/` failed on the working tree** (`util.py`, `dar_backup.py`) — an unused `import re` inside `add_specific_archive_completer()` and two lines exceeding the 150-character limit (the `--doc` argument definition and the restore-test `dar -x` command list) were left over from earlier edits.  Fixed by removing the unused import and reformatting both lines to the multi-line style already used elsewhere in the same files.  No behavior change.

- **`grep -c ... || echo 0` corrupted DIFF/INCR content-verification counts into a two-line value whenever a count was genuinely zero** (`scripts/large_scale_test.sh`) — `grep -c` already prints `"0"` (with exit code 1) when there are no matches; the `|| echo 0` fallback, intended only to keep `set -e` from aborting the script on that non-zero exit, unconditionally appended a *second* `"0"` line whenever `grep -c` exited non-zero.  The resulting `"0\n0"` value then failed the `[[ "$count" -ge N ]]` comparison with a bash syntax error instead of a clean pass/fail.  Exposed by a real smoke test against a tiny synthetic dataset where the count was legitimately zero; masked in every prior real run because the real ~116GB dataset always produced at least one match.  Fixed in both `verify_diff_contents` and `verify_incr_contents` by wrapping the grep as `{ grep -c ... || true; }`, which preserves the `set -e` guard without printing anything extra.

- **`write_json_record()` could report "failed to write structured JSON record" even when the write succeeded** (`scripts/large_scale_test.sh`) — the function's last statement was `[[ -n "$repo_dir" ]] && info "..."`; when there was nothing to mirror, that expression's own `false` result became the function's return status, which the call site's `write_json_record || echo "WARNING: ..."` then reported as a failure — despite the JSONL record having already been written correctly.  Pre-existing but only manifested once the new `--smoketest` flag (above) made "nothing to mirror" a legitimate, common case rather than a rare one (`REPO_DIR` unset).  Fixed with an explicit `return 0` at the end of the function.

- **`add_specific_archive()`'s chronological safety check bypassed `CommandRunner`** (`manager.py`) — the "is this archive older than the latest cataloged one?" check called raw `subprocess.run()` directly instead of `_runner().run()`, so it had no timeout (could hang indefinitely on a stuck `dar_manager`), no `LC_ALL=C` normalization, and nothing was written to the command log — unlike every other `dar_manager` invocation in this file.  Fixed by routing the check through `_runner().run(["dar_manager", "--base", database_path, "--list"], timeout=timeout)`, the same pattern already used by `relocate_archive_paths()` and others; the `subprocess.CalledProcessError`/`OSError` handlers collapse into a single `if result.returncode != 0` check since `CommandRunner` never raises on spawn failure.  The now-unused `import subprocess` was removed.  Five existing tests updated to mock `runner.run()` instead of `subprocess.run()`; one new test asserts the configured `COMMAND_TIMEOUT_SECS` is passed through.

- **Invalid backup-definition names failed inconsistently depending on how they were discovered** (`dar_backup.py`) — a name that fails `_normalize_backup_definition_name()`'s validation (see [naming rules](../doc/config-reference.md#backup-definition-example)) exited `rc=1` when passed explicitly via `-d`, but only `rc=2` (a warning) when found while walking `BACKUP.D_DIR` for an all-definitions run — the same validation failure had two different severities depending on invocation mode, which matters for exit-code-based monitoring.  Both paths now exit `rc=1`: a backup definition that never ran is a real failure, not a warning, consistent with the existing "definition does not exist" preflight check which already hard-fails with `exit(127)`.  **Behavior change**: if `BACKUP.D_DIR` contains non-definition files today (a stray README, `.gitkeep`, editor backup, etc.) that were previously skipped with just a logged warning, an all-definitions run will now report `FAILURE` instead — see the [exit codes reference](../doc/cli-reference.md#dar-backup-exit-codes).  Two new tests cover the mixed-directory case (invalid file skipped, valid definitions still processed) and confirm an all-valid directory is unaffected.

- **`--venv`/`--dar-path` were embedded unescaped in a single-quoted shell string** (`dar_backup_systemd.py`) — `build_exec_command()` interpolates both values into `ExecStart=/bin/bash -c '{exec_command}'`; a value containing a literal single quote would silently break the generated systemd unit.  Not attacker-exploitable — you supply your own paths when generating your own units — but a real correctness gap.  Fixed by adding `_reject_if_shell_unsafe()`, called on both values right after argument parsing and before anything is written to disk; it raises a clear error and exits 1 rather than writing a broken unit file.  Four new tests cover the accept/reject cases for both `--venv` and `--dar-path`.

- **Three `except Exception: pass`/`continue` blocks silently discarded diagnostic information** (`util.py`, `manager.py`) — a logging-handler close, a stdout-encoding reconfigure, and a catalog-listing date-parse failure each swallowed their exception with no log line at all, found by enabling `S110`/`S112`.  Fixed by adding a `logger.debug(...)` call naming the exception before the existing `pass`/`continue`, consistent with this project's "never silently swallow exceptions" convention; no behavior change, only diagnosability.

- **A misconfigured Discord webhook URL (e.g. a stray `file://` path) was passed straight to `urllib.request.urlopen()` with no scheme check** (`util.py`, found by enabling `S310`) — `send_discord_message()` now rejects any `DAR_BACKUP_DISCORD_WEBHOOK_URL` that does not start with `http://` or `https://`, logging an error and returning `False` before constructing the request.  One new test asserts `urlopen` is never called for a non-http(s) URL.

- **Preflight `--version` health checks for `dar`/`par2`, and two argcomplete tab-completion helpers, invoked `dar_manager`/`par2` by bare name, relying on `$PATH` resolution inside the subprocess call** (`dar_backup.py`, `util.py`, found by enabling `S607`) — all four sites now resolve the full path via `shutil.which()` first (three of them already called `shutil.which()` for an existence check but discarded the result) and exec the resolved absolute path instead. Same behavior when the binary is missing (`shutil.which()` returns `None`/falls back to the bare name, exactly as before); slightly hardened against `$PATH` manipulation when the binary is present.

- **Two `verify()` failure paths raised bare `Exception` instead of the project's own `BackupError`** (`dar_backup.py`, found by enabling `TRY002`) — a failed archive integrity test and a failed per-file restore-test command both did `raise Exception(str(process))`, even though `BackupError` (`util.py`) is used throughout the rest of this same file for exactly this kind of failure, and the function's own docstring already documented `BackupError` in its `Raises:` section. Both now raise `BackupError`; both are still caught by the existing generic `except Exception` handlers further up, so there is no behavior change, only a more specific and greppable exception type.

- **`sanitize_cmd()`'s type-check failures raised `ValueError`, the same type used for its content-safety check** (`command_runner.py`, found by enabling `TRY004`) — a non-list `cmd` or a non-string element is a caller programming error (a `TypeError`), not a value problem, and conflating the two made it impossible for a caller to distinguish "you passed the wrong shape" from "you passed a dangerous argument". The two type-check branches now raise `TypeError`; the two call sites that catch this exception were widened from `except ValueError` to `except (ValueError, TypeError)` so the existing graceful degradation (return a `CommandResult(returncode=-1, ...)` instead of crashing) is preserved. Two new tests assert the `TypeError` type directly.

- **~40 `except` blocks logged only `logger.error(...)`, discarding the traceback** (`dar_backup.py`, `manager.py`, `cleanup.py`, `command_runner.py`, `util.py`, `clean_log.py`, found by enabling `TRY400`) — converted to `logger.exception(...)`, which captures the full traceback automatically; a further ~20 of these had also embedded the exception object directly in the message text (found by enabling `TRY401`), which is now redundant since `logger.exception()` already prints it as part of the traceback — those messages were trimmed back to their non-redundant context (e.g. `"PREREQ failed: %s", exc` → `"PREREQ failed"`).  Pure diagnostic-quality improvement; no behavior change.

- **PITR `--restore-path` silently fell back to a single-archive restore, producing stale content and missing files, whenever a backup definition's `-R` was anything other than `/`** (`manager.py`, reported externally while smoke-testing the companion container tool `dar-backup-image`) — `_is_directory_path()` hardcoded `os.path.isdir(os.sep + path)`, unconditionally assuming every backup was taken with `-R /`. For any other `-R` (e.g. `-R /data`), this fast filesystem check looked in the wrong place and misdetected a genuine directory as "not a directory". `_detect_directory()`'s archive-catalog fallback only inspects the most recent FULL archive, so it could not rescue a directory introduced later (in a DIFF or INCR) from this mis-detection either — and `dar_manager -f <path>` *does* track directory paths, so the code silently proceeded down the single-archive "most recent version" restore path instead of erroring out. Since a differential/incremental archive never carries forward files unchanged since its reference, that single-archive restore reproduced exactly the reported symptom: a modified file showed the wrong (older) archive's content, and files unchanged since that archive (including one half of a hardlinked pair) were missing entirely. Fixed by threading the backup definition's actual `-R` root through `_is_directory_path()`/`_detect_directory()` (both now accept a `root` parameter, correctly resolving relative catalog paths against it instead of `/`); the root is looked up via a new shared helper, `util.get_backup_definition_root()` (also now used by `dar_backup.py::verify()`, replacing its equivalent inline parsing — pure dedup, no behavior change there), with a safe `/`-assuming fallback and a logged warning if the backup definition can't be read. `get_backup_definition_root()` also strips a surrounding quote pair from the `-R` value — confirmed empirically that dar's own `-B` reference-file parser requires a `-R` value containing a space to be quoted (unquoted, dar splits it into multiple invalid targets), so a real-world `-R` with a space needs this to round-trip correctly. New tests: unit coverage for `get_backup_definition_root()` and the `root`-aware `_is_directory_path()`/`_detect_directory()` (including `-R` values with spaces and non-ASCII/UTF-8 characters), plus a real (non-mocked) integration test building an actual FULL→DIFF→INCR chain under a non-`/`, space-and-UTF-8-containing `-R` and asserting the restored directory matches byte-for-byte with the hardlink preserved — confirmed via the fail-then-pass pattern to actually fail (reproducing the exact "stale content" symptom) against the pre-fix code.

- **`large_scale_test.sh` had never tested a non-`/` `-R`, so this release gate would not have caught the `_is_directory_path()` bug above either** (`scripts/large_scale_test.sh`) — five places (`write_backup_def()`'s auto-appended `-g` line, `verify_diff_contents()`, `verify_incr_contents()`, the Phase 3a `manager --restore-path` call, and the `RESTORE_PRIMER_PATH` reconstruction) all stripped exactly one leading slash off `DIFF_PRIMER_DIR` (`${DIFF_PRIMER_DIR#/}`), which is only correct if `-R` is literally `/`. Rephrased around a new `--backup-root` flag (default `/data`, matching the reported bug's own example and `BASE_DIR`'s existing default) rather than hardcoding a *different* single root: all five sites now strip `"$BACKUP_ROOT/"` instead of `/`, and `write_backup_def()` fails fast with a clear error if the caller's `--definition` `-R` doesn't match `--backup-root`, instead of silently producing a wrong restore three phases later. Separately, `check_disk_space()`'s own `-R`/`-g` extraction (`grep -m1 '^-R ' "$def_file" | awk '{print $2}'`) only captured the first whitespace-separated field — confirmed broken against a real quoted `-R`/`-g` value containing a space (`awk '{print $2}'` on `-R "path with space"` yields the bogus `"path`). Fixed by using the now-validated `$BACKUP_ROOT` directly for `-R` (no need to re-parse a value the fail-fast check already confirmed) and a small quote-aware Python parser mirroring `get_backup_definition_root()`'s surrounding-quote-strip for `-g` lines. `SCRIPT_VERSION` bumped `7` → `8`.
