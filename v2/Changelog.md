...
- **Direct archive restores could silently overwrite populated or protected targets** (`dar_backup.py`, `restore_target_safety.py`) — direct `--restore` always passed DAR `-wa`, but unlike PITR it did not validate, lock, or inspect an explicit `--restore-dir`; no `--selection` restored the whole archive and `--selection="-g ."` explicitly selected its root. Both restore paths now use one policy-driven target-safety context that independently controls protected locations and existing-data behavior. Direct restores require an empty, non-protected target; PITR root restores require empty targets; PITR specific paths reject only overlaps. Internal policy coverage exercises every protected-target × existing-data combination, including dormant allow-overwrite/protected-target policies without exposing new CLI options. Raw direct selection can no longer inject `-R`, batch files, or command execution.
