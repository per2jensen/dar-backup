...
def test_send_discord_message_prefers_config_over_env(monkeypatch):
    captured = {}
    ...
    monkeypatch.setenv("DISCORD_WEBHOOK_URL", "https://env.example/webhook")
    config = SimpleNamespace(discord_webhook_url="https://config.example/webhook")

    assert util.send_discord_message("hello", config_settings=config, timeout_seconds=3) is True
    assert captured["url"] == "https://config.example/webhook"
...
def test_send_discord_message_uses_env_when_no_config(monkeypatch):
    ...
    monkeypatch.setattr(util.urllib.request, "urlopen", fake_urlopen)
    monkeypatch.setenv("DISCORD_WEBHOOK_URL", "https://env-only.example/webhook")
...
def test_send_discord_message_live(monkeypatch):
    ...
    if not webhook:
        pytest.skip("DISCORD_WEBHOOK_URL not set for live Discord test")
