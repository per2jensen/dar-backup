...
# set INC_AGE calc to fail
sed -i s/INC_AGE.*/INC_AGE=2MUST-FAIL/ "$TESTDIR"/conf/dar-backup.conf

"$TESTDIR"/bin/cleanup.sh --local-backup-dir

COUNT=$(grep -c -E "ERROR.*?INC_AGE_DATE" "$TESTDIR"/archives/dar-backup.log)
echo "COUNT: $COUNT"
if [[ "$COUNT" == "0" ]]; then
  echo \"FAIL\" INC_AGE_DATE error message not found
  TEST_RESULT=1
fi
